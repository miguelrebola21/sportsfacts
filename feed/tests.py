...
class FeedViewTests(TestCase):
    ...
    def test_card_tags_add_multiple_selected_filters(self):
        ...
        self.assertContains(response, "min-width: 100px")
        ...
        self.assertContains(response, "basketball fact")
        self.assertContains(response, "football record")
...
class XlsxImportExportTests(TestCase):
    def test_exports_facts_with_tags(self):
        tag = Tag.objects.create(name="basketball")
        fact = Fact.objects.create(text="exported fact", upvotes=2, downvotes=1)
        fact.tags.add(tag)
        ...
        self.assertEqual(rows[0], ("id", "text", "tags", "upvotes", "downvotes"))
        self.assertEqual(rows[1], (fact.id, "exported fact", "basketball", 2, 1))

    def test_imports_facts_and_creates_tags(self):
        workbook = Workbook()
        sheet = workbook.active
        sheet.append(["id", "text", "tags", "upvotes", "downvotes"])
        sheet.append([None, "imported fact", "basketball, football", 3, 1])
        content = BytesIO()
    ...
    def test_imports_records_and_tags(self):
        workbook = Workbook()
        sheet = workbook.active
        sheet.append(["id", "number", "text", "tags", "upvotes", "downvotes"])
        sheet.append([None, 232, "imported record", "football", 4, 2])
        content = BytesIO()
        workbook.save(content)
    ...
    def test_imports_global_workbook(self):
        ...
        facts_sheet.append(["id", "text", "tags", "upvotes", "downvotes"])
        facts_sheet.append([None, "global imported fact", "basketball", 1, 0])
        records_sheet = workbook.create_sheet("Records")
        records_sheet.append(["id", "number", "text", "tags", "upvotes", "downvotes"])
        records_sheet.append([None, 232, "global imported record", "basketball", 2, 1])
        content = BytesIO()
        workbook.save(content)
